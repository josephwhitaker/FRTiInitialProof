...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kForRentInitialAlloyProjectUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kForRentInitialAlloyProjectUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
