...
NSString * ForRentInitialAlloyProject$ModuleRequireFormat = @"(function(exports){"
...
@implementation ForRentInitialAlloyProjectObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_forrentinitialalloyproject);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_forrentinitialalloyproject gc];
}
...
{
	// create ForRentInitialAlloyProject global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ForRentInitialAlloyProject" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_forrentinitialalloyproject = [[ForRentInitialAlloyProjectObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_forrentinitialalloyproject];
	
	NSString *_forrentinitialalloyprojectNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _forrentinitialalloyprojectNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_forrentinitialalloyproject valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:ForRentInitialAlloyProject$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.forrentinitialalloyproject.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.forrentinitialalloyproject.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
